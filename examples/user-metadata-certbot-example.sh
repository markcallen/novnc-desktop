...
#
# Usage:
# Pass this script as user data when launching an EC2 instance:
#   aws ec2 run-instances \
#     --image-id ami-xxxxx \
#     --user-data file://user-metadata-certbot-example.sh \
#     --environment CERTBOT_DOMAIN=myapp.example.com \
#     --environment NOVNC_PORT=443 \
#     --environment NOVNC_UNSECURED_PORT=80
#
# Or via EC2 user data UI, set environment variables before the script:
#   #!/bin/bash
...
NOVNC_SERVICE_NAME="${NOVNC_SERVICE_NAME:-novnc-server}"
...
# Step 1: Install certbot and certbot DNS plugin(s) if needed
...
# Step 4: Update novnc-server configuration
...
# Step 5: Create or update systemd drop-in to use the certificate
...
# Step 6: Restart novnc-server with new configuration
log "Restarting $NOVNC_SERVICE_NAME..."

systemctl daemon-reload
systemctl restart "$NOVNC_SERVICE_NAME" || error "Failed to restart $NOVNC_SERVICE_NAME"
...
# Step 7: Set up automatic certificate renewal via certbot cron
...
#!/bin/bash
# Restart novnc-server after certificate renewal
systemctl restart novnc-server || logger -t novnc-certbot "Failed to restart novnc-server after cert renewal"
HOOK_EOF
